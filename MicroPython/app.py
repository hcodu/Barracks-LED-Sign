...
def handle_request(method, path):
    ...
    if path.startswith("/effect/"):
        ...
        return "200 OK", f"power={'on' if power_on else 'off'} effect={current_effect} speed={speed_pct}% brightness={brightness_pct}% ip={ip}"
...
def poll_server():
    try:
        ...
        method, path, raw_data = parse_request(client)
        if method == "PUT" and path == "/upload":
            if receive_upload(client, raw_data):
                send_response(client, "200 OK", "Upload OK, rebooting...")
                server_socket.close()
                time.sleep(0.5)
                machine.soft_reset()
            else:
                send_response(client, "500 Error", "Upload failed")
            return
        if method and path:
